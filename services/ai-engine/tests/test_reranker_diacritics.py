...
from ai_engine.core.providers.llm.local import LexicalClient
...
def test_strip_diacritics_folds_vietnamese_marks():
    assert LexicalClient._strip_diacritics("Đăng") == "Dang"
...
def test_unaccented_query_matches_accented_kb():
    ...
    assert LexicalClient._lexical_score(unaccented, KB_TEXT) == LexicalClient._lexical_score(
        accented, KB_TEXT
    )
...
def test_unaccented_query_scores_well_above_floor():
    ...
    score = LexicalClient._lexical_score("Khong dang nhap duoc may tinh", KB_TEXT)
...
def test_tokenize_is_case_and_accent_insensitive():
    assert LexicalClient._tokenize("Đăng Nhập") == LexicalClient._tokenize("dang nhap")


def test_unrelated_text_still_scores_low():
    """Folding accents must not make everything match everything."""
    assert LexicalClient._lexical_score("May in bi ket giay", KB_TEXT) < 0.45
