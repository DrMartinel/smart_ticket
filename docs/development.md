...
Prompts are versioned files in `services/ai-engine/src/ai_engine/llm/prompts/`. Bump the version in the filename and in `config.py`, and note what changed. **Prompt changes go through the eval gate exactly like code changes** — that is the entire reason `evals/` lives in this repo and runs in CI.
